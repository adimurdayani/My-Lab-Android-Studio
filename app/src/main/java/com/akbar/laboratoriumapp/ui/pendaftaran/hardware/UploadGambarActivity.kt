...
class UploadGambarActivity : AppCompatActivity() {
    private var _binding: ActivityUploadGambarBinding? = null
    private val binding get() = _binding!!
    private lateinit var s: SharedPref

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        _binding = ActivityUploadGambarBinding.inflate(layoutInflater)
    ...
    private fun simpan(file: File) {
        if (s.getStatusLogin()) {
            ...
            ApiConfig.instanceRetrofit.uploadFoto(user.nim, fileImage!!)
                .enqueue(object : Callback<ResponseModel> {
                    override fun onResponse(
                        ...
                        if (response.isSuccessful) {
                            ...
                            pushAcitivty(UploadBuktiTransaksiActivity::class.java)
                        } else {
                            binding.progresBar.visibility = View.GONE
